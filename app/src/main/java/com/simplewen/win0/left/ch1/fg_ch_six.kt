...
class fg_ch_six: Fragment(){
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View {
        ...
        var vi = inflater.inflate(R.layout.fg_ch_six_1,container,false)
        when(key){
            0 -> {
                vi = inflater.inflate(R.layout.fg_ch_six_1,container,false)
                val web = vi.findViewById<WebView>(R.id.fg_ch_six_web_1)
                webLoad(web,url_ch_three[0])
            }
            1 -> {
                vi = inflater.inflate(R.layout.fg_ch_six_2,container,false)
                val web = vi.findViewById<WebView>(R.id.fg_ch_six_web_2)
                webLoad(web,url_ch_three[1])
            }
            2 -> {
                vi = inflater.inflate(R.layout.fg_ch_six_3,container,false)
                val web = vi.findViewById<WebView>(R.id.fg_ch_six_web_3)
                webLoad(web,url_ch_three[2])
            }
            3 -> {
                vi = inflater.inflate(R.layout.fg_ch_six_eg,container,false)
                val web = vi.findViewById<WebView>(R.id.fg_ch_six_web_eg)
